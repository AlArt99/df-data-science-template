...
outlier_columns = list(df.iloc[:, :6])
category_columns = list(df.iloc[:, -4:-1])

for column in category_columns:
    df[outlier_columns[:3] + [f"{column}"]].boxplot(
        by=f"{column}", layout=(2, 3), figsize=(20, 10)
    )
    df[outlier_columns[3:] + [f"{column}"]].boxplot(
        by=f"{column}", layout=(2, 3), figsize=(20, 10)
    )
...
dataset, _, _ = mark_outliers_lof(df[df["label"] == "bench"], outlier_columns)
for col in outlier_columns:
    plot_binary_outliers(
        dataset=dataset, col=col, outlier_col="outlier_lof", reset_index=True
    )
...
for col in outlier_columns:
    for label in df["label"].unique():
        ...
        print(f"Label: {label}, column: {col}, n_outliers: {n_outliers}")
